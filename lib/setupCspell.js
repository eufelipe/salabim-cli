...
async function setupEnvironmentConfig() {
  ...
  const { configureMultipleEnvs } = await prompts({
    ...
    message: "Do you want to configure multiple environment files (e.g., production, development, staging)?",
    initial: true,
  });
...
`;
    ...
    // Update package.json scripts for multiple environments
...
`;
  ...
  if (commitAnswer.commit) {
    ...
    shell.exec(`git add ${filesToAdd.join(" ")}`);

    const commitMessage = configureMultipleEnvs
      ? 'chore: setup environment configuration with react-native-config and multiple environments'
      : 'chore: setup environment configuration with react-native-config';
